...
describe('Open Pamoja AT', function() {
	it('Open and login to Pamoja AT', function() {
		console.log('enka1');
		// browser1.windowHandleFullscreen();
		browser1.url('https://at.staging.pamojaedu.com/');

		console.log(browser1.log('server'));

		var emailInput = browser1.$('[data-e2e="at__login-email-input"]'); 
		emailInput.waitForVisible();

		emailInput.setValue('martin.kregar+all@dlabs.si');
		$('[data-e2e="at__login-pass-input"]').setValue('Test1234.');

		// Wait for button to become enabled
		var loginButton = browser1.$('[data-e2e="at__login-submit-button"]');
		loginButton.waitForEnabled();
		loginButton.click();

		// Wait the page to load
		browser1.waitForVisible('h1=Courses');

		browser1.checkViewport();
	});
});


describe.skip('Create course', function() {
